...
function treinar(treino,dTreino,nTreino,valida,dValida,nValida,nEntradas,nEscondida,nSaida,eta1,eta2,nEpocas,nomePesos);
...
a1 = sqrt(3/nEntradas);  
a2 = sqrt(3/nEscondida); 

% a1 = 0.1;                                   % Faixa de variação dos pesos iniciais da camada escondida
% a2 = 0.1;                                   % Faixa de variação dos pesos iniciais da camada de saida
...
while (epocas<=nEpocas)
  ...
  [w1,w2,saturados(epocas)]=backprop(treino,dTreino,nTreino,nEscondida,nEntradas,nSaida,eta1,eta2,w1,w2);

  % Validacao cruzada
  [erro(epocas)]=testar(valida,dValida,nValida,w1,w2);
